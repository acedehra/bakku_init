 ...
 */
export function validateHeaders(headers: KVEntry[]): ValidationResult {
  const errors: string[] = [];
 ...
 */
export function validateCustomAuth(headerName: string, headerValue: string): ValidationResult {
...
export function validateAuth(authConfig: {
  ...
  switch (authConfig.type) {
    ...
    case 'Custom':
      return validateCustomAuth(authConfig.headerName || '', authConfig.headerValue || '');
 ...
 */
export function validateRequestBody(body: string, contentType?: string): ValidationResult {
  // Empty body is valid
  if (!body) {
    return { isValid: true };
  }
 ...
 */
export function validateParams(params: KVEntry[]): ValidationResult {
  const errors: string[] = [];
