...
export class RequestCache<T = unknown> {
  ...
  constructor(
    defaultTtl: number = cacheConfig.ttlMs,
    enabled: boolean = cacheConfig.requestCaching
  ) {
    this.cache = new Map();
 ...
 */
export function cacheRequest(url: string, value: unknown, options?: CacheOptions): void {
 ...
 * Get cached request
 */
export function getCachedRequest<T>(url: string, options?: CacheOptions): T | null {
  const key = defaultCache['generateKey']?.(url, options) ?? url;
...
export function invalidateCache(url: string): void {
  ...
  stats.keys.filter((key) => key.startsWith(url)).forEach((key) => defaultCache.delete(key));
}
