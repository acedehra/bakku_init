...
import { requestConfig, featureFlags } from '../config';
...
function calculateRetryDelay(
  attempt: number,
  baseDelay: number = requestConfig.retryDelayMs
): number {
...
export const buildAuthHeaders = (
  ...
  if (authConfig.type === 'Basic' && authConfig.username && authConfig.password) {
    ...
    const credentials = btoa(
      `${encodeURIComponent(substitutedUsername)}:${encodeURIComponent(substitutedPassword)}`
    );
...
export async function executeHttpRequest(
  ...
  for (const h of headers) {
    ...
    finalHeaders.push([
      substituteVariables(h.key, activeEnv),
      substituteVariables(h.value, activeEnv),
    ]);
  }
  ...
  if (canHaveBody && substitutedBody.trim()) {
    ...
    if (!requestOptions.headers.some(([k]) => k.toLowerCase() === 'content-type')) {
      requestOptions.headers.push(['Content-Type', 'application/json']);
    }
  }
  ...
  const enableCache = options?.enableCache ?? featureFlags.requestCaching;
  // Check cache for GET requests only if caching is enabled
  ...
  // Execute request with retry logic (if maxRetries > 0)
  const effectiveRetries = options?.maxRetries ?? requestConfig.maxRetries;
  const res =
    effectiveRetries > 0
      ? await executeWithRetry(
          async () => tauriFetch(substitutedUrl, requestOptions),
          (response) => RETRYABLE_STATUS_CODES.includes(response.status),
          effectiveRetries
        )
      : await tauriFetch(substitutedUrl, requestOptions);
  ...
  if (contentType.includes('application/json')) {
    try {
      ...
      formattedBody = json !== null ? JSON.stringify(json, null, 2) : '(empty JSON)';
    } catch {
  ...
  if (enableCache && method === 'GET' && res.status >= 200 && res.status < 300) {
