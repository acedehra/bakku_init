...
describe('FolderItem', () => {
  ...
  it('calls onStartRenameFolder when Rename is clicked in context menu', () => {
    ...
    expect(defaultProps.onStartRenameFolder).toHaveBeenCalledWith(mockFolder, expect.any(Object));
  });
  ...
  it('calls onDeleteFolder when Delete is clicked in context menu', () => {
    ...
    expect(defaultProps.onDeleteFolder).toHaveBeenCalledWith('folder-1', expect.any(Object));
  });
  ...
  it('shows rename input when renaming', () => {
    render(
      <FolderItem {...defaultProps} renamingFolderId="folder-1" renameValue="Renamed Folder" />
    );
