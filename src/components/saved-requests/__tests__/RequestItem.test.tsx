...
describe('RequestItem', () => {
  ...
  it('applies selected styling when selected', () => {
    const { container } = render(<RequestItem {...defaultProps} selected={true} />);
  ...
  it('calls onStartRename when Rename is clicked in context menu', () => {
    ...
    expect(defaultProps.onStartRename).toHaveBeenCalledWith(mockRequest, expect.any(Object));
  });
  ...
  it('shows rename input when renaming', () => {
    render(<RequestItem {...defaultProps} renaming={true} renameValue="Renamed Request" />);
  ...
  it('renders different HTTP methods correctly', () => {
    ...
    methods.forEach((method) => {
      const { unmount } = render(
        <RequestItem {...defaultProps} request={{ ...mockRequest, method }} />
      );
